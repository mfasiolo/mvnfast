...
When `d` and `N` are large, and `rmvn` is called several times with the same arguments, it would make sense to create the matrix where to store the simulated random variable upfront. This can be done as follows:
```{r rmvnA}
...
Pre-creating the matrix of random variables provide considerable speed ups: 
```{r rmvnA2}
microbenchmark(rmvn(N, mu, mcov, ncores = 2, A = A),
               rmvn(N, mu, mcov, ncores = 2), 
               times = 200)
```
Don't look at the median time here, the mean and the maximum time are much more affected by memory re-allocation. 
